...
class StudentQModel():
        ...
        def load(self, filename):
                self.model = DQN.load(filename, self.model.env)

        def basicPolicy(self, x,y,theta):
            best_a = self.model.predict([x,y,theta])[0]
            return self.env.actions[best_a]

        def strict_q_policy(self, x, y, theta):
            best_q = -float('inf')
            best_a = None
            for a in self.env.actions:
                q = self.q_value(x, y, theta, a)
                if q >= best_q:
                    best_a = a
                    best_q = q
            return best_a
...
def plot_r_q_ep(model, **kwargs):
    return r_q_ep(model, **kwargs)

def plot_reward(reward, theta=0.0, a=0.0, **kwargs):
    if 'ax' not in kwargs:
        kwargs['ax'] = plt.gca()
    cs = xy_contourf(reward, theta, a, **kwargs)
    kwargs['ax'].set_title('R(x, y, θ={}, a={})'.format(theta, a))
    plt.colorbar(cs)
    return kwargs['ax']
